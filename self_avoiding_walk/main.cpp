...
int main() {
  ...
  for (int j = 0; j < ROWS; j++) {
    for (int i = 0; i < COLS; i++) {
      grid.push_back(
          std::make_unique<Cell>(sf::Vector2f(i, j), PATH_SIZE, ROWS, COLS));
    }
  }
  ...
  while (window.isOpen()) {
    ...
    if (count <= n_denied) {
      ...
      if (current) {
        lines.append(sf::Vertex(sf::Vector2f(current->position.x * PATH_SIZE,
                                             current->position.y * PATH_SIZE),
                                sf::Color::Red));
      }

    } else if (!bufferOperationDone) {  // Only clear and repopulate the lines
                                        // once
      lines.clear();
